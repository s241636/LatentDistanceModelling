...
for undirected in [True, False]:
    for subset_percent in [0.1, 0.3, 0.5]:
        for k in [1,2,3,5,10,20,30,40,50]:
            loader = DataLoader()
            constructor = GraphConstructor()
            X, y = loader.load_MNIST(subset_percent=subset_percent)
            adjacency_matrix = constructor.construct_knn(X, k_neighbors=k, undirected=undirected)
            ...
            for epoch in range(epochs):
                ...
                if epoch % 10 == 0:
                    print(
                        f"Epoch {epoch:4d} | Loss: {loss.item():.4f} | Alpha: {ldm.alpha.item():.4f}"
                    )
