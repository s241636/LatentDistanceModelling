...
def load_mammoth(subset_percent: float = 1.0) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Load the mammoth CSV and return (X, y).

    The dataset currently has no explicit label column, so this returns a
    dummy label tensor with shape `(N, 1)` to keep downstream evaluation code
    working without special cases.
    """
    mammoth = pd.read_csv("data/mammoth.csv")
    mammoth = mammoth.to_numpy()
    if subset_percent == 1.0:
        return torch.tensor(mammoth), torch.zeros((len(mammoth), 1), dtype=torch.long)
    n = len(mammoth)
    indices = np.random.randint(0, n, size=round(n * subset_percent))
    return torch.tensor(mammoth[indices]), torch.zeros((len(indices), 1), dtype=torch.long)
...
def load_pendigits(subset_percent: float) -> tuple[torch.Tensor, torch.Tensor]:
    # Load Pendigits and keep the class label in the last column.
    pendigits = pd.read_csv("data/pendigits.csv")
    data = pendigits.to_numpy()
    X_full = data[:, :-1].astype(np.float32) / 100.0
    y_full = data[:, -1].astype(int)
    if subset_percent == 1:
        return torch.tensor(X_full), torch.tensor(y_full)
